...
#   Summarize input parameters
    ...
    print(f"    linear_harmonics         :{linear_harmonics:>20}")
    print(f"    linear_torsions          :{linear_torsions:>20}")
    ...
    print(f"    MD_temperature           :{MD_temperature:>20}")
    print(f"    MD_dt                    :{MD_dt:>20}")
    print(f"    MD_stepsize              :{MD_stepsize:>20}")
    print(f"    MD_startup               :{MD_startup:>20}")
    print(f"    MD_energy_lower_cutoff   :{MD_energy_lower_cutoff:>20}")
    print(f"    MD_energy_upper_cutoff   :{MD_energy_upper_cutoff:>20}")
    if(method == "cMMMD"):
    ...
    print(f"    modSem                   :{modSem:>20}")
    if(modSem):
        print(f"    modSem_finite_step       :{modSem_finite_step:>20}")
        print(f"    modSem_vib_scaling       :{modSem_vib_scaling:>20}")
        print(f"    modSem_tolerance         :{modSem_tolerance:>20}")
#   Convert dataset sizes to "per-conformer"
    Ntrn = int(Ntrn / Ncnf)
    Ntst = int(Ntst / Ncnf)
#   parameterize the molecule and output the forcefield to a file
    ...
    trainable, topology = build_parameters(mol,VdW_forcefield,ML_path,linear_harmonics,linear_torsions,modSem,modSem_finite_step,modSem_vib_scaling,modSem_tolerance) 
...
# Generate the Energy Scatter Plot
    energy_mean, energy_SD = write_scatter(dataset_test, trainable.force_field, topology, "default.scat")
...
# Generate the Energy Scatter Plot and summarize convergence
        energy_mean_new, energy_SD_new = write_scatter(dataset_test, trainable.force_field, topology, "trained-" + str(iteration) + ".scat")
        tqdm.write(f"Convergence Summary for Iteration {iteration+1}:")
        tqdm.write(f"    Energy Difference (Mean): {energy_mean:.5f}->{energy_mean_new:.5f} : Change = {energy_mean_new - energy_mean:.5f}")
        tqdm.write(f"    Energy Difference (SD):   {energy_SD:.5f}->{energy_SD_new:.5f} : Change = {energy_SD_new - energy_SD:.5f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate",            type=float, help="Learning Rate in the ML fit",                required=False, default=0.01)
    ...
    parser.add_argument("--MD_dt",                    type=float, help="MD Stepsize in picoseconds",                 required=False, default=0.001)
    parser.add_argument("--MD_energy_lower_cutoff",   type=int,   help="Lower bound for the energy cutoff function", required=False, default=1.0)
    parser.add_argument("--MD_energy_upper_cutoff",   type=int,   help="Upper bound for the energy cutoff function", required=False, default=10.0)
    ...
    parser.add_argument("--modSem_finite_step",       type=float, help="Finite Step for the Hessian Calculation",    required=False, default=0.0005291772)
